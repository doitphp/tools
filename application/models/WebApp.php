...
class WebAppModel {
	...
	public function createIndexFile() {
		...
		$filePath = rtrim(WEB_APP_PATH, '/') . DIRECTORY_SEPARATOR . 'index.php';
...
EOT;
	...
	public function createHtaccessFile() {
		...
		$filePath    = rtrim(WEB_APP_PATH, '/') . DIRECTORY_SEPARATOR . '.htaccess';
		$fileContent = <<<EOT
RewriteEngine on
...
EOT;
	...
	public function createRobotsFile() {
		...
		$filePath = rtrim(WEB_APP_PATH, '/') . DIRECTORY_SEPARATOR . 'robots.txt';
		$fileContent = <<<EOT
User-agent: *
Crawl-delay: 10
Disallow: /doitphp/
...
EOT;
	...
	protected function _createConfigFile() {
		...
		$filePath = rtrim(WEB_APP_PATH, '/') . DIRECTORY_SEPARATOR . 'application/config/application.php';
		$fileContent = <<<EOT
<?php
/**
 * 项目主配置文件
...
EOT;
	...
	public function createLogDir($isApache = false) {
		...
		$dirPath = rtrim(WEB_APP_PATH, '/') . DIRECTORY_SEPARATOR . 'logs';
