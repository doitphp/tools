...
$(function(){
	$(".fancybox").fancybox();
	$('#class_name_box').cookieBind();
	$('#class_tabname_status_box').cookieBind();
	$('#class_table_name_select').cookieBind();
	...
	$('#class_tabname_status_box').click(function(){
		if($(this).attr('checked')=='checked'){
			$('#class_table_name_box').show();
		}else{
			$('#class_table_name_box').hide();
		}
	});
});
...
function ajaxFormRequest(){
	var className=$('#class_name_box').val();
	if(className==''){
		alert('对不起，class Name不能为空！');		
		$('#class_name_box').css('border-color', '#C00').focus();
		return false;
	}
}
function ajaxFormResponse(data){
	alert(data.msg);
	if(data.status==true){
		$.cookies.del('class_name_box');
		$.cookies.del('class_tabname_status_box');
		$.cookies.del('class_table_name_box');
		$.cookies.del('note_description_box');
		$('#create_class_form_box').resetForm();
		location.reload();
	}
}
</script>
<script type="text/javascript">
	$(document).ready(function(){
		$('#create_class_form_box').ajaxForm({beforeSubmit:ajaxFormRequest,success:ajaxFormResponse,dataType:'json'});
	});
</script>
<fieldset>
<legend>创建Class文件：</legend>
<?php $this->widget('ReturnDir'); ?>
<form action="<?php echo $this->getActionUrl('ajaxcreateclass'); ?>" method="post" name="create_class_form_box" id="create_class_form_box">
<table border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
<caption>基本信息</caption>
  <tr>
    <td align="left"><label>Class Name:</label>&nbsp;&nbsp;&nbsp;&nbsp;
	<input type="text" class="text" name="class_name_box" id="class_name_box" style="width:180px;"/>
	</td>	
  </tr>
</table>
<!-- method -->
...
<table border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td height="40" align="center"><input type="submit" style="width:130px; height:24px; text-align:center; border:1px solid #333; cursor:pointer" value="创建class文件" name="create_class_submit_button"/><input type="hidden" name="path_box" value="<?php echo $path; ?>" /></td>
  </tr>
</table>
</form>
</fieldset>